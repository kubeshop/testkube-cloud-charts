...
yq eval -i '.testkube-cloud-api.image.tag = env(enterprise_api_app_version)' "charts/testkube-cloud-api/values.yaml"
echo "Updating testkube-cloud-ui version in testkube-cloud-ui values.yaml to $enterprise_ui_app_version"
yq eval -i '.testkube-cloud-ui.image.tag = env(enterprise_ui_app_version)' "charts/testkube-cloud-ui/values.yaml"
